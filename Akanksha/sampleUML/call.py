...
class Endpoint(Resource):
    ...
    def SetUp(self, context, result):
        ...
        if self.ostype == 'Linux':
                ...
                self.shell.assertCommand('sudo ip addr add %s/24 dev %s > /dev/null 2>&1' \
                                % (self.ipAddr, self.ethport))
        elif self.ostype == 'SunOS':
                self.ethport = "e1000g%s" % port
                self.shell.assertCommand( 'sudo ifconfig %s addif %s/24 up > /dev/null 2>&1' \
    ...
    def CleanUp(self, result):
        self.log.debug('Cleaning endpoint resource: property: %s, ip: %s, mask: %s, eth: %s' % \
                                  (self.property, self.ipaddr, self.mask, self.ethport))
        if self.ostype == 'Linux':
                self.shell.assertCommand('sudo ip addr del %s/24 dev %s > /dev/null 2>&1' \
                                % (self.ipAddr,self.ethport))
        elif self.ostype == 'SunOS':
