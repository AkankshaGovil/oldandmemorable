                                ...
                                H245_H2250LogicalChannelParameters & param) const
{
  ...
  if (codec != NULL) {
    ...
    if (codec->IsDescendant(H323AudioCodec::Class()) && channel.GetDirection() != H323Channel::IsReceiver) {
    ...
    int rtpPayloadType = codec->GetMediaFormat().GetPayloadType();
                                   ...
                                   H245_H2250LogicalChannelAckParameters & param) const
{
  ...
  H323Codec * codec = channel.GetCodec();
  if (codec != NULL) {
    int rtpPayloadType = channel.GetCodec()->GetMediaFormat().GetPayloadType();
    if (rtpPayloadType >= RTP_DataFrame::DynamicBase) {
