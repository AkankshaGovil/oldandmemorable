...
#------------------------------------------------------------------------------------#
# FP 51770 | Modify to hash on a unique result for (results_provision.txt)           #
# Feb 2008 |                                                                         #
#   JMO    |                                                                         #
#------------------------------------------------------------------------------------#
# FP 35605 | Initial creation of this script generateResults.sh. Compiles results    #
# Feb 2007 | into a comma separated variable *.csv file: NexTest_Results_4.0c3-63.csv#
#   JMO    |                                                                         #                   
#------------------------------------------------------------------------------------#
...
# FP 51770 Begin
# Use "sort -u" to make the result for (results_provision.txt) unique
# provisionFile=`grep production_components.provision * | grep -v -i binary | grep -v dat | cut -d':' -f1`
provisionFile=`grep production_components.provision * | grep -v -i binary | grep -v dat | cut -d':' -f1 | sort -u`
# FP 51770 End
# FP38869 Begin
