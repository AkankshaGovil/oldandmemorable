...
class ResDB extends DBEngine {
	...
	*/
	function get_resource_data($machid) {
        ...
        #Added by Nabanita
	/**
	* Returns all data about the machines of a particular resource
	* @param string $machid id of resource to look up
	* @return array of all machine data
	*/
	function get_machine_data($machid) {
                $return = array();

                $sql = 'SELECT * FROM ' . $this->get_table(TBL_MACHINES) . ' WHERE machid=?';

                $result = $this->db->query($sql, array($machid));
