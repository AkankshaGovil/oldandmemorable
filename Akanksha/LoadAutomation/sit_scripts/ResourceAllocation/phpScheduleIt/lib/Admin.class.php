...
class Admin {
	...
	*/
	var $tools = array (
					...
					'machines'	=> array ('Manage Machines', 'manageMachines', 'machines')
					);
	var $pager;
	var $db;
	var $tool;
	...
	*/
	...
	function manageMachines() {
		$this->listMachinesTable();
		$this->editMachineTable();
	}


	function listMachinesTable() {
		$pager = $this->pager;
                $num = $this->db->get_num_admin_recs('machines');      // Get number of records
                $pager->setTotRecords($num);                            // Pager method calls
                $orders = (isset( $_GET['order'] )  ? array( trim( $_GET['order'] ), 'machID' ) : array('name', 'machID'));

                $machines = $this->db->get_all_resource_data($pager, $orders);

		print_manage_machines($pager, $machines, $this->db->get_err());
                $pager->printPages();	
	}

	
	function editMachineTable() {
		...
		print_machine_edit($rs, $scheds, $edit, $this->pager);
		unset($_SESSION['post'], $rs);
	}	
