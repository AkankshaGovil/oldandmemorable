...
function print_manage_machines(&$pager, $machines, $err) {
	global $link;
        $util = new Utility();

?>

<form name="manageMachine" method="post" action="admin_update.php" onsubmit="return checkAdminForm();">
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
    <td class="tableBorder">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr>
          <td colspan="8" class="tableTitle">&#8250; <?php echo translate('All Machines')?></td>
        </tr>
                <?php echo "
        <tr class=\"rowHeaders\">
          <td>" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'name'), translate('Machine Name')) . "</td>
          <td width=\"18%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'ip'), translate('Machine IP')) . "</td>
                  <td width=\"12%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'id'), translate('Machine ID')) . "</td>
          <td width=\"10%\">" . translate('MediaCardType') . "</td>
    ...
    for ($i = 0; is_array($machines) && $i < count($machines); $i++) {
                $cur = $machines[$i];
        echo "<tr class=\"cellColor" . ($i%2) . "\" align=\"center\" id=\"tr$i\">\n"
            . '<td style="text-align:left">' . $cur['name'] . "</td>\n"
            . '<td style="text-align:left">' . (isset($cur['ip']) ?  $cur['ip'] : '&nbsp;') . "</td>\n"
            . '<td style="text-align:left">' . $cur['id'] . "</td>\n"
...
* @param object $pager Pager object
*/
function print_machine_edit($rs, $scheds, $edit, &$pager) {
        global $conf;
        $start = 0;
        $end   = 1440;
    ...
    ?>
<form name="addMachine" method="post" action="admin_update.php" <?php echo $edit ? "" : "onsubmit=\"return checkAddResource(this);\"" ?>>
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
    <td class="tableBorder">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr>
          <td width="200" class="formNames"><?php echo translate('Machine Name')?></td>
          <td class="cellColor"><input type="text" name="name" class="textbox" value="<?php echo isset($rs['name']) ? $rs['name'] : '' ?>" />
          </td>
        </tr>
        <tr>
          <td class="formNames"><?php echo translate('Machine IP')?></td>
          <td class="cellColor"><input type="text" name="MachineIP" class="textbox" value="<?php echo isset($rs['MachineIP']) ? $rs['MachineIP'] : '' ?>" />
          </td>
        </tr>
        <tr>
          <td class="formNames"><?php echo translate('Media Card Type')?></td>
          <td class="cellColor"><input type="text" name="MCType" class="textbox" value="<?php echo isset($rs['MCType']) ? $rs['MCType'] : '' ?>" />
          </td>
        </tr>
        <tr>
          ...
          <td class="cellColor"><textarea name="notes" class="textbox" rows="3" cols="30"><?php echo isset($rs['notes']) ? $rs['notes'] : '' ?></textarea>
                ...
                <tr>
                <td class="formNames"><?php echo translate('Test Bed')?></td>
                <td class="cellColor">
                <select name="TestBedid" class="textbox">
                <?php
		for ($i = 0; is_array($resources) && $i < count($resources); $i++) {
			$cur = $resources[$i];
//			echo '<option value="' . $cur['name'] . '"' . ((isset ($rs['TestBedid']) && ($rs['TestBedid'] == $cur['name'])) ? ' selected="selected"' : '') . '>' . </option> . "\n";
	//		echo '<option value="' . $cur . '"' . '>' . </option> . "\n";	
	//		echo '<"option value="' . $resources[$i] . '" ' selected="selected"' : '') . '</option>' . "\n";
		}	
                //if (empty($scheds))
