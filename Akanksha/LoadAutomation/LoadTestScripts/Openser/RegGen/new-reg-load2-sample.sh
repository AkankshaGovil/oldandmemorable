...
getConfigVal()
{
	...
	do
           ...
           then
              ...
              then
                 iface=$val
              elif [ $param == "reg_gen_ip" ]
              then
                 SourceIp=$val
              elif [ $param == "remote_ip" ]
              then
                 RemoteIp=$val
              else
...
#extra start
	...
	then
		sip=$SourceIp
		rip=$RemoteIp
                echo "sipp -sf XML_FILE_SER_1 -i $sip $rip -inf /tmp/csv-nated$start.txt -d $delay -r $rate -users $maxnumber -nostdin -bg -trace_err -default_behaviors -bye,-abortunexp -pause_msg_ign"
	else
		sip=$SourceIp
		rip=$RemoteIp
                ...
                echo "./sipp -sf XML_FILE_SER_2 -i $sip $rip -inf /tmp/csv-nated$start.txt -d $delay -r $rate -users $maxnumber -nostdin -bg -trace_err -default_behaviors -bye,-abortunexp -pause_msg_ign"
	fi
#extar end
