...
%fields = (
  ...
  fixedsize  => undef, 		##Flag to indicate that MAX data is fixed 
...
sub add_data {
  ...
  foreach (@_) {
    ...
    if (!( $self->fixedsize() && ( $self->count() >= $self->fixedsize() ) )) { 
      $self->{sum} += $_;
      $self->{count}++;
      if ($_ >= $max) {
        $max = $_;
      ...
      $newval = $_;
      $oldval = shift @{ $self->{data} };
      $maxdex--;
      $mindex--;

      $oldmean = $self->{mean};
      $self->{sum} += $newval-$oldval;
		##    $self->{count}++; 	Count remains unchanged
      ...
      if ($newval >= $max) {
        ...
        ## Our max just flew out of the window
        ## Calculate again
        @arr = @{ $self->{data} };
        $max = $arr[0];
        $maxdex = 0;
      ...
      ## Our min just flew out of the window
      ## Calculate again
        @arr = @{ $self->{data} };
        $min = $arr[0];
        $mindex = 0;
      ...
      $self->{mean} += ($newval - $oldval) / $self->{count};
      $self->{pseudo_variance} += ($newval - $oldval)*( ($newval - $self->{mean})+ ($oldval - $oldmean));
    }
  }
...
##
## gets data low <= num <= high
##
sub get_freq_over_range {
  my $self = shift;
  my $element;
  my $num = 0;
  return undef if $self->{count} < 2; #Must have at least two elements
  ...
  my $lowrange = shift;
  my $highrange = shift;
  if ($lowrange < $self->{min}) 
  {
	$lowrange = $self->{min};
  }
  if ($highrange > $self->{max})
  {
	$highrange = $self->{max};
  }

  ELEMENT: foreach $element (@{$self->{data}}) {
      if (($element >= $lowrange) && ($element <= $highrange)) {
	$num++;
        next ELEMENT;
      }
  }
