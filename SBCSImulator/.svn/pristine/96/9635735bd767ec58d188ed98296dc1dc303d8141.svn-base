...
if [ "$MC" = "SunOS" ]; then
	...
	LDLIBS="-lpthread -lfl -lposix4 -lsocket -lnsl -lodbc -lares"
	...
	TARGET=`arch`
	PLATFORM=ppc
	PERL5LIB="$BASE/Library/Perl:$BASE/System/Library/Perl"
else
...
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:.:$BASE/lib/$TARGET:$BASE/pkgs/def/lib:$BASE/pkgs/$TARGET/lib
...
if [ -f $BASE/osd/configure -a ! -x $BASE/osd/configure ]; then
...
if [ "$PERLVERSION" = "5.10.0" ]; then 
    PERLADJVERSION="5.6.1"
    PERLOLDVERSION="5.6.0"

elif [ "$PERLVERSION" = "5.8.3" ]; then
	PERLADJVERSION="5.6.1"
	PERLOLDVERSION="5.6.0"
...
export CSCOPE_DB
export LUA_PATH="./?.lua;$BASE/psfmods/services/?.lua"
export LUA_CPATH="./?.so;$BASE/lua/lib/?.so"
