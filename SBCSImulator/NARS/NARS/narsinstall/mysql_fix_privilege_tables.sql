...
##############################################################################
## USE THIS SCRIPT WHEN UPGRADING FROM 3.22 and below to a later mysql version
##############################################################################
...
#/usr/local/mysql/bin/mysql -f --user=root --password="$root_password"  --host="$host" mysql <<END_OF_DATA
...
#  /usr/local/mysql/bin/mysql --user=root --password="$root_password" --host="$host" mysql <<END_OF_DATA
...
#/usr/local/mysql/bin/mysql --user=root --password=$root_password mysql <<EOF
