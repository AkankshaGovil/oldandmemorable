...
#define CRYPT_MAGIC_1	0xa9
#define CRYPT_MAGIC_2	0x24
...
static unsigned XOR [BLOCKSIZE] = {0x2c, 0xe8, 0x73, 0xf1, 0x38, 0xec, 0x49, 0x4b,
								   0xd9, 0xcb, 0xa9, 0xb4, 0x90, 0xda, 0x5b, 0xde} ;
...
static I32
nextone_decrypt(pTHX_ int idx, SV *buf_sv, int maxlen)
{
	...
	/* been applied on the same line as the use NexTone::decrypt	*/
...
#ifdef FDEBUG
    if (fdebug)
	warn("**** In nextone_decrypt - maxlen = %d, len buf = %d idx = %d\n", 
...
#ifdef FDEBUG
	if (fdebug)
	    warn("  nextone_decrypt(%d): sub-filter returned %d: '%.999s'",
...
MODULE = NexTone::decrypt	PACKAGE = NexTone::decrypt
